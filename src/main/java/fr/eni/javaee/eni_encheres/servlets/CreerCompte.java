	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (telephone.isBlank()) {
				...
				Utilisateur newUtilisateur = new Utilisateur(pseudo, nom, prenom, email, LoginManager.getInstance().getBase64Password(password), newAdresse);
				UtilisateurManager.getInstance().createNewUtilisateur(newUtilisateur);
			} else {
				Adresse newAdresse = new Adresse(rue, Integer.valueOf(codePostal), ville);
				AdresseManager.getInstance().createNewAdresse(newAdresse);
				Utilisateur newUtilisateur = new Utilisateur(pseudo, nom, prenom, email, telephone, LoginManager.getInstance().getBase64Password(password), newAdresse);
				UtilisateurManager.getInstance().createNewUtilisateur(newUtilisateur);
			}
